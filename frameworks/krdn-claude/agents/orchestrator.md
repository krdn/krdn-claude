...
GitHub MCP 서버가 연동되어 있어 다음 도구들을 활용할 수 있습니다.

### 사용 가능한 MCP 도구

| 도구 | 용도 | 예시 |
|------|------|------|
| `mcp__github` | GitHub API 통합 | PR, Issue, 코드 검색 |

### Pull Request 관련 작업

1. **PR 생성 시**
   - 변경사항 분석 후 PR 제목/본문 자동 생성
   - `gh pr create` 명령 활용 (Bash 도구)
   - 관련 Issue 자동 연결

2. **PR 검토 시**
   - 코드 변경 내역 분석
   - code-reviewer 에이전트와 연계
   - 리뷰 코멘트 작성

### Issue 관련 작업

1. **Issue 조회**
   - 프로젝트별 열린 Issue 목록 확인
   - 우선순위/라벨별 필터링
...
```
사용자: "claude-code-auto에 새 기능 PR 만들어줘"

1. 변경사항 분석 (git diff)
2. PR 제목/본문 생성
3. gh pr create 명령 실행
4. 결과 URL 반환
```

```
사용자: "모든 프로젝트의 열린 이슈 보여줘"

1. 프로젝트 목록 로드 (projects.json)
2. 각 프로젝트별 gh issue list 실행
3. 통합 이슈 목록 보고
```

### 주의사항

- GitHub 토큰이 `GITHUB_TOKEN` 환경변수로 설정되어야 함
- Private 저장소 접근 시 적절한 권한 필요
- Rate limit 고려하여 대량 요청 시 간격 유지
